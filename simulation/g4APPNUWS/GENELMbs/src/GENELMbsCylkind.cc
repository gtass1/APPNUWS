...
//                         boost::shared_ptr<ReadOut> &ros,
                         CylShpType boxShType
               ):
    _id(id)
//    ,_detailIndex(detailIndex)
    ,_shapeType(boxShType)
    ,_pos(pos)
//    ,_MidPoint(center)
...
//    _ros=ros;
  ...
  void Cylkind::setPosMatrix( HepGeom::Transform3D &lPos ) {
...
//  const CLHEP::Hep3Vector& Cylkind::getROChLoclMidPoint(unsigned long id)  const {
