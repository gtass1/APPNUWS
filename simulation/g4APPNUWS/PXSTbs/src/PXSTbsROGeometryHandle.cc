...
ROGeometryHandle::ROGeometryHandle( /*Detector *slt*/) :
    _fLayer(-1)
   ,_fPhiSec(-1)
   ,_fLadder(-1)
   ,_fLadderID(-1)
...
ROGeometryHandle::~ROGeometryHandle() {}
...
//  if (Layer!=_fLayer) {
//    _fLayer = Layer;
//    _lay = pxstdetector->getLayer(_fLayer);
//    change = true;
//  }
//  if (PhiSec!=_fPhiSec) {
//    _fPhiSec = PhiSec;
...
boost::shared_ptr<ReadOut> ROGeometryHandle::GetROChannel() {
	return _lad->getReadOut();
}
