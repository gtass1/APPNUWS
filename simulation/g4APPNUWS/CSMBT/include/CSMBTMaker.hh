...
#ifndef CSMBTMaker_hh
#define CSMBTMaker_hh
...
public:
  ...
  CSMBTMaker( crd::SimpleConfig const& config );
  ~CSMBTMaker ();

  // This is the accessor that will remain.
  std::unique_ptr<CSMBtracker> getCSMBTrackerPtr() { return std::move(_lpst); }
  std::unique_ptr<CSMBTAbsorber> getCSMBTabsorberPtr() { return std::move(_lpsabs); }
...
private:
  ...
  int _geomType;                //Version number (see CSMBTracker for definition)
...
//  double _z0;                   //Shift along z of the center of the tracker
  ...
  std::unique_ptr<CSMBtracker> _lpst;
  std::unique_ptr<CSMBTAbsorber> _lpsabs;
