...
namespace csmbt {
  ...
  CSMBTLadderSD::CSMBTLadderSD(const G4String &name, const G4String& hitsCollectionName, const crd::SimpleConfig& config) :
...
//
//          if ( !geom->hasElement<CSMBtracker>() ) {
//                  throw cet::exception("GEOM")
//                  << "Expected I Trackers but found neither.\n";
//          }

    if ( !GeomService::Instance()->hasElement<CSMBtracker>() ) {
      exc::exceptionG4 e("GEOM","Fatal Error",0);
      e<< "Expected CSMBT Trackers but found neither.\n";
      e.error();
    }
  }

  CSMBTLadderSD::~CSMBTLadderSD(){ }

  G4bool CSMBTLadderSD::ProcessHits(G4Step* aStep, G4TouchableHistory* ){
...
//                                          << _currentSize << endl;
                          ...
                          e<< "CSMBT: Maximum number of particles reached "
                              << SensitiveDetectorName
                              << ": "
                              << _currentSize << endl;
                          e.error();
...
//          string volName = aStep->GetTrack()->GetVolume()->GetName();
          int volCpNum = 0;
          _roSubShell = 0;
          pxstbs::Layer* tmpLayer = csmbtracker->getLayer(_layer);
...
//            volCpNum = aTouch->GetCopyNumber(1);
          ...
          bool isFw = (tmpLayer->getLayerZone()==pxstbs::Layer::forward);
          if (isFw) {
            _phiSec = volCpNum;
...
//          cout<<"volCpNum "<<volCpNum<<" Sec "<<_phiSec<<" lad "<<_ladder<<endl;
          ...
          if ( verboseLevel>=2 )  std::cout<<"CSMBT: G4 hit pos in World "<<prePosWorld[0]<<" "<<prePosWorld[1]<<" "<<prePosWorld[2]<<std::endl;
          G4ThreeVector prePosTracker = prePosWorld - _csmbtDetCenter;
          G4ThreeVector postPosTracker = aStep->GetPostStepPoint()->GetPosition() - _csmbtDetCenter;
          ...
          CSMBTROGeometryHandle *roghndl = csmbtracker->getROGeometryHandle();
...
//            ((SimG4TrackerHit*)fHitsCollection->GetHit(fHitsCollection->entries()-1))->SetChannelNb(channel);
